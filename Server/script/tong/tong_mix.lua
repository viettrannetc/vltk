...
function MAP_CREAT_G_1(nTongID, nMapCopy)
	...
	if (nTongMap > DYNMAP_ID_BASE) then
		...
		return 0
	elseif (nMapCopy == 0 and nTongLevel < 5) then
		if(nTongMap > 0)then
			...
			local aNum = {
				...
				gb_GetTask("TONG_PUBLIC_MAP", 5),
				gb_GetTask("TONG_PUBLIC_MAP", 6),
				gb_GetTask("TONG_PUBLIC_MAP", 7),
				}
			Say("<#>HiÖn ®¼ng cÊp kiÕn thiÕt d­íi cÊp 2, chØ cã thÓ dïng khu vùc chung, h·y chän 1 khu vùc chung (nªn dïng khu vùc cã Ýt bang héi):  ", 8,
			
			"S¬n Thµnh (®· cã "..aNum[1].." bang héi)/#PublicMap("..nTongID..",1)", 
			"Miªu C­¬ng (®· cã "..aNum[2].." bang héi)/#PublicMap("..nTongID..",2)",
			"Sa M¹c (®· cã "..aNum[3].." bang héi)/#PublicMap("..nTongID..",3)", 
			"Kinh S­ (®· cã "..aNum[4].." bang héi)/#PublicMap("..nTongID..",4)",
			"Biªn ¶i (®· cã "..aNum[5].." bang héi)/#PublicMap("..nTongID..",5)",
			"Giang Nam (®· cã "..aNum[6].." bang héi)/#PublicMap("..nTongID..",6)",
			"TuyÕt LÜnh (®· cã "..aNum[7].." bang héi)/#PublicMap("..nTongID..",7)",
			"Hñy bá /#Cancel"
			)
			return 0
		end
...
function STUNT_SET_R(nTongID, nStuntID)
	local szExecutorName = TONGM_GetName(nTongID, ExecutorId)
	if (TONG_GetBuildLevel(nTongID) < 2) then
