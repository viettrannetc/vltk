...
function tbSeed:Check()
	...
	if (offlineCheckPermitRegion() == 0) then
		...
		elseif (GetLevel() < 50) then
			...
			return 0
		--elseif (tbSeed:GetPlantCount() >= 10) then
			--Say(format("H«m nay %s ®· tham gia ®ñ 10 lÇn råi!", mate_name))
			--return 0
...
if (nRandomKeyValue < 50) then
i = 3455
end
if
(nRandomKeyValue <= 80 and nRandomKeyValue>=50) then
i = 3456
end
if
(nRandomKeyValue <= 95 and nRandomKeyValue>80) then
i = 3457
end
if
(nRandomKeyValue <= 100 and nRandomKeyValue>95) then
i = 3458
end

		self.AwardFlag[Name] = 1
		tbAwardTemplet:Give(tbAward, 1, {"VuonCaySinhNhat", "NhanPhanThuong["..tbTreeName[Tree.Kind].."]"})		
		tbAwardTemplet:GiveAwardByList({tbProp = {6,1,i,0,0,0}, nCount = 1, nExpiredTime=1440}, "test", 1);
