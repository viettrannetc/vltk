...
-----¹ºÂò¹¥³ÇÕ½µÀ¾ß start
function AskDeal()
	Say("Dông cô hç trî c«ng thµnh chiÕn cùc kú hiÖu dông, chØ cã thÓ dïng Kim Nguyªn B¶o ®Ó mua! Kh¸ch quan nÕu bá lì kh«ng mua th× thËt v« cïng ®¸ng tiÕc!", 4, 
	--"»ëÊ¯ÁÑ/DealBuy", 
...
function DoBuy()
	...
	if (TheItem >= 29 and TheItem <= 32) then
		if (GetItemCountEx(343) > 0) then
			...
			WriteLog(format("%s dïng kim nguyªn b¶o ®Ó mua 1 ®¹o cô c«ng thµnh >> %s",GetName(),TheItem));
...
function check_award_condition(city_index, show_talk)
	if (city_index == 0) then
		if (show_talk == 1) then
			Talk(1, "", "ChØ cã bang ph¸i chiÕm ®­îc thµnh thŞ míi nhËn ®­îc phÇn th­ëng, h·y dÉn huynh ®Ö cña m×nh ®i chiÕm mét thµnh nµo ®i!")
		end
		return 0
	end
...
function take_tong_award(count)
	...
	if (CalcFreeItemCellCount() < count) then
		Talk(1, "", "Hµnh trang cña ng­¬i ®· ®Çy, h·y s¾p xÕp l¹i hµnh trang råi h·y ®Õn l·nh th­ëng.")
		return
	end
