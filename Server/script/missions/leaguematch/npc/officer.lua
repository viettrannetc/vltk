...
function main1()
	...
	Say("CÊp 120 míi cã thÓ tham gia <color=yellow>Vâ l©m liªn ®Êu<color>, luyÖn lªn cÊp 120 råi quay l¹i ®©y!", 0);
	return 
	end
	...
	local tb_option = {}
	tb_option[getn(tb_option) + 1] = "Ta muèn ®Õn héi tr­êng "..str_des.."/wlls_want2signmap"
...
function wlls_wantaward()
	...
	tb_option[getn(tb_option)+1] = "Ta muèn ®æi phÇn th­ëng Vinh dù/wlls_buy_honour"
...
function wlls_wantaward_title()
	...
	if (FALSE(n_lid) or n_rank <= 0 or n_rank > 4) then
		Say(wlls_npcname().." Gi¶i th­ëng nµy chØ tÆng "..wlls_get_desc(1).." cho nh÷ng ng­êi xuÊt s¾c nhÊt (4 ®éi h¹ng ®Çu) . Ng­¬i kh«ng thuéc nhãm ®ã, ®õng ®Õn ®©y ph¸ rèi!", 0)
		return nil
	end
	local n_sid = GetGlbValue(GLB_WLLS_SID)
	local str_lgname = LG_GetLeagueInfo(n_lid)
...
function wlls_cancreate(n_level)
	...
	if (n_mylevel ~= n_level) then
		if (n_level == 1) then
			if (n_mylevel < n_level) then
				wlls_descript("<enter>  <color=red>Vâ l©m kiÖt xuÊt<color> tuy nãi lµ ®Ó chän ra nh÷ng hµo kiÖt ­u tó cña c¸c m«n ph¸i trong giang hå, nh­ng ®¼ng cÊp cña b¹n vÉn ch­a ®ñ! "..WLLS_LEVEL_JUNIOR.." cÊp, vâ nghÖ ch­a tinh, chØ sî ®ao kiÕm v« nh·n, lµm ng­¬i bŞ th­¬ng. Ng­¬i nªn tiÕp tôc luyÖn tËp!")
			else
				wlls_descript("<enter>  C¸c h¹ ®· lµ ®¹i hiÖp giang hå ®Ønh ®Ønh ®¹i danh, <color=red>Vâ l©m kiÖt xuÊt<color> lµ ®Ó chän ra nh÷ng tµi n¨ng trÎ, ng­¬i cÇn g× ph¶i ra oai víi ®¸m hËu bèi chø? Hay lµ ng­¬i tham gia <color=red>Vâ l©m liªn ®Êu<color> ®i!")
			end
		else
...
function wlls_want2signmap()
	...
	if (FALSE(n_lid)) then
		...
		if (WLLS_TAB[n_type].max_member == 1) then
			wlls_descript("B¹n ch­a cã chiÕn ®éi t¹i <color=yellow>"..str_des.."<color> , nÕu tham gia thi ®Êu, hÖ thèng sÏ lËp cho b¹n 1 chiÕn ®éi. B¹n cã muèn lËp 1 chiÕn ®éi kh«ng?", "Ta muèn lËp chiÕn ®éi!/#wlls_createleague()", "Ta kh«ng muèn thi ®Êu!/OnCancel")
		else
