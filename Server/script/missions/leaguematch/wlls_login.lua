	...
	if (n_lid ~= 0) then
		...
		if (n_phase >= 2) then
			...
			if (n_wday == 0 or n_wday >= 5) then
				Msg2Player("HiÖn ®ang trong gian ®o¹n <color=yellow>Vâ l©m liªn ®Êu<color>, h«m nay tõ <color=yellow>20:00~21:00<color> vµ <color=yellow>22:00~23:00<color> cã tiÕn hµnh <color=yellow>8<color> trËn "..WLLS_LEVEL_DESC[n_level]..".")
			else
				Msg2Player("HiÖn ®ang trong gian ®o¹n <color=yellow>Vâ l©m liªn ®Êu<color>, h«m nay tõ <color=yellow>20:00~21:00<color> vµ <color=yellow>22:00~23:00<color> cã tiÕn hµnh <color=yellow>8<color> trËn "..WLLS_LEVEL_DESC[n_level]..".")
			end
		else
		...
		if (n_level > 0) then
			Msg2Player("B¹n cã thÓ ®Õn gÆp Quan viªn b¸o danh tham gia <color=yellow>Vâ l©m liªn ®Êu<color>.")
		end
	end
