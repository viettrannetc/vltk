...
function pActivity:GiveAward()
	...
	local tbAward = {
		...
		[1]={nExp_tl=3e6},
		[2]={szName="<color=yellow>Hçn Nguyªn Linh Lé",tbProp={6,1,2312,1,0,0},nCount=1},
		[3]={szName="<color=yellow>TiÒn §ång", tbProp={4,417,1,1,0,0,0}, nCount = 20},	
	}
	tbAwardTemplet:Give(tbAward, 1, {%EVENT_LOG_TITLE, "NhËn phÇn th­ëng tõ Th­¬ng Bu«n"})
--	local msg = format("<color=green>Chóc mõng cao thñ <color=yellow>%s<color=green> ®· nhËn ®­îc <color=yellow><%s><color=green> tõ <color=yellow><VËn Tiªu><color>" ,GetName(),GetItemName(szName))	
	str = "<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· nhËn ®­îc phÇn th­ëng hoµn thµnh nhiÖm vô VËn Tiªu"
	AddGlobalCountNews(str, 2)
	Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().." ®· nhËn ®­îc hoµn thµnh nhiÖm vô VËn Tiªu")
