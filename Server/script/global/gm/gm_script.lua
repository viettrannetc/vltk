...
function tbAloneScript:DialogMain()
	local szTitle = format("<npc>Chµo mõng <color=red><player><color> tham gia m¸y chñ <servername>, ®­îc ph¸t triÓn bëi <dev>! §©y lµ lÖnh bµi hç trî dµnh cho GM ®Ó qu¶n lý, ®iÒu hµnh m¸y chñ cña m×nh.");
	local tbOption = {};
		tinsert(tbOption, {"TÝnh n¨ng hç trî thö nghiÖm", self.SupportGMTestGame})
...
function tbAloneScript:SupportGMTestGame()
	local tbSay = {"<dec><npc>Mêi GM tr·i nghiÖm chøc n¨ng trong game"};
...
function tbAloneScript:SkillsSystem()
	local szTitle = "<npc>HÖ thèng kü n¨ng bao gåm thªm kü n¨ng vµ xãa kü n¨ng, b¹n muèn sö dông hÖ thèng kü n¨ng nµo?"
	local tbOpt = {}
