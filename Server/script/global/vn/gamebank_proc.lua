...
function get_ingot()
	...
	if (nMoney <= 0) then					-- Ã»ÓĞÀ©Õ¹µãÊı
		...
		local nCurVar = nMoney *COIN
		Say("<#> Kh¸ch quan ®· göi t¹i bæn trang  tæng céng <color=green>"..nCurVar.."<color> TiÒn §ång. Cã muèn lÊy ra kh«ng?", 3, 
"Ta muèn lÊy ra "..COIN.." TiÒn §ång/get_ingot10",
"Ta muèn lÊy ra 100 TiÒn §ång/get_ingot100",
"Ta muèn lÊy ra 1000 TiÒn §ång/get_ingot1000",
 "Kh«ng lÊy/no")
	end
end
------------------10 tien-----------
function get_ingot10()
	...
	Say("Kh¸ch quan x¸c ®Şnh muèn rót  TiÒn §ång chø? (Sau khi x¸c nhËn xin ®õng tho¸t khái trß ch¬i!) ", 2, 
"X¸c nhËn/get_ingot10ok", 
"Hñy bá /no");
end
...
function get_ingot100()
	...
	Say("Kh¸ch quan x¸c ®Şnh muèn rót  TiÒn §ång chø? (Sau khi x¸c nhËn xin ®õng tho¸t khái trß ch¬i!) ", 2, 
"X¸c nhËn/get_ingot100ok", 
"Hñy bá /no");
end
function get_ingot1000()
	if (SYSCFG_GAMEBANK_SILVER_GET == nil) then -- ³ö´íÁË~~
		print("get_silver have close, but something is wrong...");
		Msg2Player("Chøc n¨ng bŞ lçi!");
		return
