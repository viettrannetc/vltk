-- TÝnh n¨ng: Söa chöa trang bÞ h­ háng - Editor by AloneScript (Linh Em)
...
function sure10()
	if (CalcEquiproomItemCount(4, 417, 1, 1)>=40) then 
		Talk(1,"","Ha! Ha! §· nhËn ®­îc 40 TiÒn §ång. §©y lµ <color=red>B¨ng Tµm V« Cùc T¬<color>. §¹i hiÖp xem! §¶m b¶o cã thÓ th¨ng cÊp y phôc, kh«i m·o!")
	  	...
	  	AddEventItem(415)
	  	UseSilver(1, 2, 2); -- Sè liÖu thèng kª tiªu thñ cña KNB trao ®æi víi tiÒn ®ång (tiªu thô trùc tiÕp thái vµ vÐ b¹c, t­¬ng ®­¬ng víi trao ®æi tiÒn ®ång)
	  	SaveNow(); -- L­u ngay lËp tøc
...
function deal_brokenequip()
	...
	return end
	Say("B¹n cã <color=red><trang bÞ tæn h¹i><color>kh«ng? Muèn söa nh­ thÕ nµo?", 2, "Ta muèn sö dông TiÒn ®ång ®Ó kh«i phôc l¹i/restore_brokenequip", "ta kh«ng cÇn ®©u! H·y gi÷ l¹i ®i!/throw_brokenequip");
end
...
function on_restore_be(nCount)
	...
	if (nQuality == 0) then
		nNeedCoin = 1;
	elseif (nQuality == 1) then -- Trang bÞ hoµng kim
		nNeedCoin = 5;
	elseif (nQuality == 2) then -- Trang bÞ mµu tÝm
		nNeedCoin = 2;
	elseif (nQuality == 4) then -- Trang bÞ b¹ch kim
		nNeedCoin = 10;
	end
	...
	if (PayCoin(nNeedCoin) == 1) then
		if (ITEM_BrokenEquip2Normal(nItemIdx) == 1) then
			SetCurDurability(nItemIdx, GetMaxDurability(nItemIdx)) -- Kh«i phôc ®é bÒn tèi ®a
