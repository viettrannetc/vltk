...
function check_zhuansheng()
	...
	--if (ntranscount ~= 0) then
		--Phong V©n LÖnh Bµi - §iÒu chØnh thêi gian trïng sinh ®èi víi c¸c t©n thñ - Modified By DinhHQ - 20110926
		--******************Fix NamCungNhatThien bá thêi gian tiÕp theo trung sinh************
	...
	if tbPVLB_Check:IsNewPlayer() == 1 and tbPVLB_Check:CheckTime() == 1 then
		if ntranscount == 0 then
		 	if GetCash() < 50000000 then
				CreateTaskSay({"H×nh nh­ tiÒn vÉn ch­a ®ñ <color=red>50000000 <color>.", "§­îc råi./OnCancel"});
				return 0;
			end
		elseif ntranscount == 1 then
			if GetCash() < 1000000000 then
				CreateTaskSay({"H×nh nh­ tiÒn vÉn ch­a ®ñ <color=red>1000000000 <color>.", "§­îc råi./OnCancel"});
				return 0;
			end
		else
